...
\begin{itemize}
    ...
    \end{equation}
    \indent gdzie $i$ - liczba klas w procesie klasyfikacji, $TPR_i$ - miara Precission obliczona dla i-tej klasy, $w_i$ - liczba dokumentów z i-tej klasy, zatem $\sum_{i=1}^{n}w_i$ oznacza liczbę wszystkich klasyfikowanych dokumentów.
    \item Dla Recall wzór wygląda analogicznie, jak powyżej:
    \begin{equation}
        PPV_{waz} = \frac{\sum_{i=1}^{n}w_iPPV_i}{\sum_{i=1}^{n}w_i}
    \end{equation}    
    \indent gdzie $i$ -liczba klas w procesie klasyfikacji, $PPV_i$ - miara Recall obliczona dla i-tej klasy, $w_i$ - liczba dokumentów z i-tej klasy, zatem $\sum_{i=1}^{n}w_i$ oznacza liczbę wszystkich klasyfikowanych dokumentów.
\end{itemize}
...
\begin{equation}
\label{eqn:trigramy}
    sim_2(s_1, s_2) = \frac{1}{N - 2}\sum_{i=1}^{N - 2}h(i)\
\end{equation}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item K najblższych sąsiadów: ???
        \item Proporcja podziału zbioru: ???
        \item Metryka: ???
        \item Wybrane cechy: ???
    \end{itemize}
    Miara Accuracy w tym przypadku wyniosła:???
...
\begin{figure}[H]
    \centering
    \includegraphics[width=15cm, height=10cm]{modul_gui.png}
    \caption{Diagram UML dla modułu GUI}
\end{figure}
// Krótki opis 
\begin{figure}[H]
    \centering
    \includegraphics[width=15cm, height=10cm]{modul_dao.png}
