...
//        String[] summarizersNames = {"test1", "test2"};
    ...
    public LinguisticSummary generateMultiSubjectSummary() {
        ...
        String firstSubject = subjectLabel1.getText().toLowerCase(Locale.ROOT)
                .replaceFirst("a", "i").replaceFirst("ia", "iej");
        String secondSubject = subjectLabel2.getText().toLowerCase(Locale.ROOT)
                .replaceFirst("a", "i").replaceFirst("ia", "iej");
        ...
        for (Node label : multiSummarizersRoot.getChildren()) {
            Label convertedLabel = (Label) label;
            summarizersText.append(convertedLabel.getText()).append("i ");
            for (LinguisticVariable variable : linguisticVariables) {
                for (LinguisticLabel linguisticLabel : variable.getLabels()) {
                    if (linguisticLabel.getName().equals(convertedLabel.getText())) {
                        summarizers.add(linguisticLabel);
        ...
        summarizersText.deleteCharAt(summarizersText.length() - 2);
        ...
        if (form == 0) {
            summaryText = quantifier.getLabel().getName() + " zawodników z " + firstSubject + " w porównaniu " +
                    "do zawodników z " + secondSubject + " jest " + summarizersText;
        } else if (form == 1) {
            summaryText = quantifier.getLabel().getName() + " zawodników z " + firstSubject + " w porównaniu " +
                    "do zawodników z " + secondSubject + ", którzy są " + qualifiersText + ", jest " + summarizersText;
        } else if (form == 2) {
            summaryText = quantifier.getLabel().getName() + " zawodników z " + firstSubject + ", którzy są " +
                    qualifiersText + ", w porównaniu " + "do zawodników z " + secondSubject + " jest " + summarizersText;
        } else {
            summaryText = "Więcej zawodników z " + firstSubject +
                    " niż z " + secondSubject + " jest " + summarizersText;
        }
